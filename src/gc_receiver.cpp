    ...
    declare_parameter<std::string>("interface_ip", ""))
{
  ...
  gc_publisher = create_publisher<oxebots_interfaces::msg::Referee>(
    get_parameter("gc_topic_name").as_string(), get_parameter("topic_retention").as_int());

  RCLCPP_INFO(get_logger(), "Game controller receiver module started");
}

GCReceiver::~GCReceiver() {RCLCPP_INFO(get_logger(), "Stopping game receiver module...");}
...
void GCReceiver::get_ball_left_field(
  const GameEvent_BallLeftField & ball_left_field, oxebots_interfaces::msg::GameEvent & game_event)
{
