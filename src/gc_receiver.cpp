                       ...
                       declare_parameter<std::string>("interface_ip", ""))
{
    ...
    gc_publisher = create_publisher<oxebots_interfaces::msg::Referee>(get_parameter("gc_topic_name").as_string(),
                                                                      get_parameter("topic_retention").as_int());

    RCLCPP_INFO(rclcpp::get_logger("GCReceiver"), "Game controller receiver module started");
}

GCReceiver::~GCReceiver() { RCLCPP_INFO(rclcpp::get_logger("GCReceiver"), "Stopping game receiver module..."); }

oxebots_interfaces::msg::TeamInfo GCReceiver::get_team_info(const Referee_TeamInfo & team)
{
    ...
    if (team.has_ball_placement_failures_reached())
        team_info.ball_placement_failures_reached = team.ball_placement_failures_reached();
    ...
    if (team.has_bot_substitution_time_left())
        team_info.bot_substitution_time_left = team.bot_substitution_time_left();

    return team_info;
}

oxebots_interfaces::msg::Vector2f GCReceiver::get_vector2(const Vector2 & vector)
{
...
void GCReceiver::get_ball_left_field(const GameEvent_BallLeftField & ball_left_field,
...
oxebots_interfaces::msg::GameEvent GCReceiver::get_game_event(const GameEvent & event)
{
    oxebots_interfaces::msg::GameEvent game_event;
    game_event.id = event.id();

    for (auto origin : event.origin())
        game_event.origin.push_back(origin);
    ...
    switch (event.event_case())
    {
        ...
        {
            get_ball_left_field(event.ball_left_field_touch_line(), game_event);
            break;
        }
        case oxebots_interfaces::msg::GameEvent::AIMLESS_KICK:
        {
            ...
            game_event.kick_location = get_vector2(event.aimless_kick().kick_location());
            break;
        }
        case oxebots_interfaces::msg::GameEvent::ATTACKER_TOO_CLOSE_TO_DEFENSE_AREA:
        {
            game_event.by_robot = event.attacker_too_close_to_defense_area().by_bot();
            game_event.location = get_vector2(event.attacker_too_close_to_defense_area().location());
            break;
        }
        case oxebots_interfaces::msg::GameEvent::PLACEMENT_SUCCEEDED:
        {
            if (event.placement_succeeded().has_time_taken())
                game_event.time = rclcpp::Time(event.placement_succeeded().time_taken());
        ...
        case oxebots_interfaces::msg::GameEvent::PENALTY_KICK_FAILED:
        {
            ...
            if (event.penalty_kick_failed().has_location())
                game_event.location = get_vector2(event.penalty_kick_failed().location());
        ...
        case oxebots_interfaces::msg::GameEvent::NO_PROGRESS_IN_GAME:
        {
            if (event.no_progress_in_game().has_location())
                game_event.location = get_vector2(event.no_progress_in_game().location());

            if (event.no_progress_in_game().has_time())
                game_event.time = rclcpp::Time(event.no_progress_in_game().time());
        ...
        case oxebots_interfaces::msg::GameEvent::PLACEMENT_FAILED:
        {
            if (event.placement_failed().has_remaining_distance())
                game_event.distance = event.placement_failed().remaining_distance();

            if (event.placement_failed().has_nearest_own_bot_distance())
                game_event.nearest_bot_distance = event.placement_failed().nearest_own_bot_distance();
...
void GCReceiver::on_receive(const Referee & packet)
{
    ...
    if (packet.has_blue_team_on_positive_half())
        gc_referee.blue_team_on_positive_half = packet.blue_team_on_positive_half();
    else
    ...
    gc_referee.game_event_proposals = std::vector<oxebots_interfaces::msg::GameEventProposalGroup>();
    ...
    if (packet.has_current_action_time_remaining())
        gc_referee.current_action_time_remaining = rclcpp::Time(packet.current_action_time_remaining());
...
void GCReceiver::PublishGCData(oxebots_interfaces::msg::Referee gc_referee) { gc_publisher->publish(gc_referee); }

int main(int argc, char * argv[])
{
    rclcpp::init(argc, argv);
    rclcpp::spin(std::make_shared<GCReceiver>());
    rclcpp::shutdown();
    return 0;
}
