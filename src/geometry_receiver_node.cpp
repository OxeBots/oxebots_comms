    ...
    declare_parameter<std::string>("interface_ip", ""))
{
    ...
    geometry_publisher =
      this->create_publisher<oxebots_interfaces::msg::SSLGeometryData>(
...
void GeometryReceiverNode::on_receive(const SSL_WrapperPacket & packet)
{
    ...
    if (packet.has_geometry())
    {
        ...
        RCLCPP_DEBUG(get_logger(),
                     "Field size: length=%d, width=%d, goal_width=%d, "
                     "goal_depth=%d, boundary_width=%d",
                     field_size.field_length, field_size.field_width,
                     field_size.goal_width, field_size.goal_depth,
                     field_size.boundary_width);

        // Log the camera calibrations
        for (const auto & calib : camera_calibrations)
        {
            RCLCPP_DEBUG(get_logger(),
                         "Camera ID %d: focal_length=%f, principal_point=(%f, "
                         "%f), distortion=%f",
                         calib.camera_id, calib.focal_length,
...
void GeometryReceiverNode::convertFieldSize(
  ...
  oxebots_interfaces::msg::SSLFieldSize & field_size)
{
    ...
    {
        field_size.center_circle_radius =
          geometry.field().center_circle_radius();
    }
    ...
    {
        field_size.goal_center_to_penalty_mark =
          geometry.field().goal_center_to_penalty_mark();
    }
...
void GeometryReceiverNode::convertCameraCalibrations(
  const SSL_GeometryData & geometry,
  std::vector<oxebots_interfaces::msg::SSLCameraCalibration> &
    camera_calibrations)
{
...
void GeometryReceiverNode::convertGeometryModels(
  const SSL_GeometryData & geometry,
  oxebots_interfaces::msg::SSLGeometryModels & geometry_models)
{
    if (geometry.has_models())
    {
        ...
        {
            oxebots_interfaces::msg::SSLBallModelStraightTwoPhase
              straight_two_phase;
            straight_two_phase.acc_slide =
              models.straight_two_phase().acc_slide();
            straight_two_phase.acc_roll =
              models.straight_two_phase().acc_roll();
            straight_two_phase.k_switch =
        ...
        if (models.has_chip_fixed_loss())
        {
            ...
            chip_fixed_loss.damping_xy_first_hop =
              models.chip_fixed_loss().damping_xy_first_hop();
            chip_fixed_loss.damping_xy_other_hops =
