...
TEST_F(GameReceiverTest, TestUdpReceivePublish)
{
  ...
  auto data_comp = std::make_unique<SSL_WrapperPacket>();
  auto detection_frame_data = std::make_unique<SSL_DetectionFrame>();
  ...
  for (int i = 0; i < 3; i++) {
    ...
    auto detection_robot_yellow = detection_frame_data->add_robots_yellow();
  ...
  data_comp->set_allocated_detection(detection_frame_data.release());
  std::string serialized_data_comp = data_comp->SerializeAsString();

  ASSERT_NE(serialized_data_comp, "");
  RCLCPP_INFO(rclcpp::get_logger("test_logger"), "Serialized data is not empty.");

  // Subscription for robot data
  oxebots_interfaces::msg::RobotPosition received_robot_data;
  auto robot_subscription = node_->create_subscription<oxebots_interfaces::msg::RobotPosition>(
    "robot_data", 10,
    [&message_robot_received,
    &received_robot_data](const oxebots_interfaces::msg::RobotPosition::SharedPtr msg) {
      received_robot_data.allies = msg->allies;
      received_robot_data.enemies = msg->enemies;
      message_robot_received = true;
      RCLCPP_INFO(rclcpp::get_logger("test_logger"), "Robot data received.");
    });

  // Subscription for ball data
  oxebots_interfaces::msg::BallPosition received_ball_data;
  auto ball_subscription = node_->create_subscription<oxebots_interfaces::msg::BallPosition>(
    "ball_data", 10,
    [&message_ball_received,
    &received_ball_data](oxebots_interfaces::msg::BallPosition::SharedPtr msg) {
      received_ball_data.x = msg->x;
      received_ball_data.y = msg->y;
      received_ball_data.z = msg->z;
      message_ball_received = true;
      RCLCPP_INFO(rclcpp::get_logger("test_logger"), "Ball data received.");
    });

  // Simulate UDP message sending
  RCLCPP_INFO(rclcpp::get_logger("test_logger"), "Sending a message...");
  udp::endpoint remote_endpoint(address::from_string("224.5.23.2"), 10006);

  boost::asio::io_context io_context;
  udp::socket test_socket(io_context, udp::endpoint(udp::v4(), 0));

  test_socket.send_to(boost::asio::buffer(serialized_data_comp), remote_endpoint);
  RCLCPP_INFO(rclcpp::get_logger("test_logger"), "UDP message sent.");

  // Wait for message reception
  auto start = std::chrono::steady_clock::now();
  while (!message_robot_received && !message_ball_received &&
